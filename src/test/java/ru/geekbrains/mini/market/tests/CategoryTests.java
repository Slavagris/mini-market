...
public class CategoryTests {
    ...
    @BeforeAll
    static void beforeAllbeforeAll() throws MalformedURLException {
        categoryService = RetrofitUtils.getRetrofit().create(CategoryService.class);
    }

    @Test
    void getFoodCategoryPositiveTestgetFoodCategoryPositiveTest()  throwsIOException {
    ...
    void getElectronicsCategoryCorrectId()  throwsIOException {
        Response<Category> response = categoryService
                .getCategory(2)
                .execute();
        assertThat(response.isSuccessful()).isTrue();
    ...
    void getFurnitureCategoryCorrectId()  throwsIOException {
        Response<Category> response = categoryService
                .getCategory(3)
                .execute();
        assertThat(response.isSuccessful()).isTrue();
    ...
    void cycleTestOfCorrectContentTypeInResponseCategories()  throwsIOException {
        for (int i = 1; i < 4; i++) {
            Response<Category> response = categoryService
                    .getCategory(i)
                    .execute();
    ...
    void getSomeNegativeTestAboutAbsentCategory()  throwsIOException {
        Response<Category> response = categoryService
                .getCategory(4)
                .execute();
    ...
    void cycleThrowCategoriesTestConnectionKeepAliveInHeader()  throwsIOException {
        for (int i = 1; i < 4; i++) {
            Response<Category> response = categoryService
                    .getCategory(i)
                    .execute();
